...
Stejně dobře se můžete radit nebo si nechat napovědět od nějakého AI systému, od obecného jako\ GPT-3/4/5, nebo specializovaného jako GitHub Copilot nebo Claude. Ten vám také dokáže naprogramovat jednoduché úkoly. Může vám také pomoci s domácími úkoly, ty ale volíme tak, aby nebyly pro GPT lehké. Snažte se, pokud to jde, programovat sami, jinak se nenaučíte zjistit, zda je generovaný kód v pořádku.
...
V tomto semestru nám už Idle nebude stačit, budeme psát složitější kódy a budeme dělat těžší chyby, takže se naučíme i nějaké techniky ladění kódu. Na přednáškách budu používat -_PyCharm_ a _VSCode_, abyste si na obě prostředí zvykli.  Uvidíte ale, že pro kratší kódy dobře vystačíte s "lehkým" vybavením pozůstávajícím z nástroje pro správu virtuálního prostředí (např. *uv*) a lehkého editoru (*Zed* nebo *Neovim*).
...
**První domácí úkoly najdete v ReCodExu po skončení tohoto cvičení.**
...
```python
# To je jenom pro ilustraci, stačí samotné reversed
...
A co kdybychom si namísto počítání vytvořili tabulku a jenom z ní odčítali součty a přenosy? Stačí slovník:

```python
long_add_table = {(i, j) : divmod(i + j, 10) fir i in range(10) for j in range(10)}
```

Pokud ale máme tabulku, vůbec nemusíme řešit konverzi argumentů na int.
