...
2. **Domácí úkoly** 
   ...
   Tento týden byly domácí úkoly "nepovinné", ale i tak přišlo docela hodně řešení
   
   1. Mrkev a petržel - správná řešení, někde chybělo alespoň krátké zdůvodnění.
   2. Výpočet postfixového výrazu - většinou jste zvládli úplně správně.
3. **Zápočtový program**:  

   - Zatím mám jeden návrh na téma zápočtového programu a jeden hotový kód na revizi. Ostatní prosím nezapomeňte na blížící se konec dubna. 
...
Python takovýto kód explicitně nezakazuje, mnohé jiné jazyky ale ano. V novějších verzích Pythonu dostanete varování `SyntaxWarning: 'return' in a 'finally' block`.
...
Toto je svým způsobem lehké, protože můžeme lehce najít rekurzivní vztah mezi počty osazení pro n záhonů a n-1 záhonů, pokud se díváme na poslední záhon u všech možných osazení:

Pokud je $m_n$ počet osazení s posledním záhonem osazeným mrkví a $p_n$ počet osazení s posledním záhonem osazeným petrželí, pak (protože ke každému osazení končícímu mrkví nebo petrželí můžeme přidat záhon mrkve a ke každému osazení končícímu mrkví můžeme přidat záhon petržele)
$$
m_{n+1} = m_{n} + p_{n} \quad p_{n+1} = m_{n}
$$
Protože celkový počet osazení je $c_n = m_n + p_n$ , dostaneme 
$$
c_{n+1} = c_n + m_n = c_n + c_{n-1}
$$
Pak už stačí jenom počáteční podmínky pro důkaz, že $c_n = F_{n+1}$, tedy řešení jsou Fibonacciovy čísla. 

Vyskytlo se jedno originální kombinatorické řešení: počet osazení o velikosti n je počet možností, jak rozmístit $k$ záhonů petržele do $n-k+1$ možných umístění mezi řádky mrkve pro všechna $k \le n-k + 1$ . Musíme počítat potenciálně mnoho kombinačních čísel, ale tam můžeme dost ušetřit, pokud použijeme rekurenci mezi kombinačními čísly.  

### Výpočet postfixového výrazu

Typické řešení je rekurentní: 
...
Můžeme také použít zásobník, ale je šíkovné vzít to od konce:

```python
import sys
from operator import add, sub, mul, floordiv

operatory = {"+": add, "-": sub, "*": mul, "/": floordiv}
symbols = input().split()
...
for symbol in reversed(symbols):
    if symbol.isnumeric():
        ...
        try:
            ...
            sys.exit()
        try:
            stack.append(operatory[symbol](op1, op2))
        except KeyError:
            print("CHYBA!")
            sys.exit()
        except ValueError:
...
Podobně lehké je dekódovat postfixovou notaci; infixová notace je ale těžší a proto se jí budeme věnovat v další části. 
