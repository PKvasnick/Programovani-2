...
3. **Domácí úkoly**: Měli jsme malou nepravidelnost, kdy jste na tento týden dostali jenom jeden nový domácí úkol. LSS máme za sebou, ale dnes ještě dostanete jeden úkol na LSS a jeden už o něčem úplně jiném.
...
Už jsme se setkali s "indikací typu" u objektů v Pythonu:
...
Toto je ale přesně kód, který nechceme: umíme sečítat objekty typu `int`, `float`, `str`, `list` atd. Pokud chceme obecný kód pro sčítání, je vyznačení typu krok přesně opačným směrem. Lepší je označení typů vůbec nepoužít. 

My ale přece jenom chceme něco podstatného říci o argumentech a návratové hodnotě sečítací funkce: všechny musí být stejného typu. 

Parametr `T` v definici funkce `add` nazýváme **typová proměnná** a je to něco jako parametr šablony v C++. Typové proměnné umožňují přesně to, co šablony v C++: psát generický kód, platný pro víc typů objektů, ale současně poskytují možnost vyjádřit omezení, kladná na použité typy:

- Můžeme vymezit, jakého typu může být proměnná T:

```
def add[T:(int, float, str)](x: T, y: T) -> T:
	return a + b
```

Ověřte si, že váš editor vyznačí chybu, pokud použijete funkci `add` pro dva seznamy. 

- Můžeme vyznačit nejvšeobecnější požadavky na typy proměnných. Tyto požadavky se nazývají "horní meze" a myslí se tím, že je splňuje vyznačený typ a všechny od něj odvozené typy:
...
U funkce `unique` potřebujeme zabezpečit, aby argument byl iterovatelný kontejner (tedy takový, pro který můžeme napsat `for x in xs`), a aby byl hashovatelný - tedy neměnný. 
...
- Analyzátor kódu i interpret brání změně atributů instancí zmrzlé třídy, i když samotnou třídu modifikovat lze: argument `frozen` ovlivňuje vytvářené instance.
...
## Příklad na LSS: tabulka pro inverzní interpolaci

 Potřebujeme vytvořit tabulku funkce pro inverzní interpolaci, tedy např. pro přibližné řešení rovnice $f(x) = a$. Proto potřebujeme, aby tabulka byla hustší tam, kde se funkce mění, zatímco na intervalech, kde se mění pomalu, vystačíme s menší hustotou. 

Konkrétně můžeme požadovat tabulku pro f(x) na intervalu <a, b> takovou, aby se na každém intervalu, kde se funkce mění o víc než $\epsilon$, nacházel alespoň jeden bod tabulky. 

![](img\LSS_II.png)

​								Funkce $y = sin(x`2)$, $\epsilon= 0.2$.



Toto není těžká úloha, dokud nezačneme přemýšlet, jakou strukturu potřebujeme pro uložení bodú tabulky. Totiž nejjednodušší by bylo začít se dvěma okrajovými body a pak začít tabulku zahušťovat:

- Vyjdeme z bodu `x[0] = a`  a počátečního nejbližšího bodu `x[1] = b`.
- Pokud $|f(a) - f(b)| > \epsilon$, přidáme doprostřed nový bod `x[2] = (a + b)/2`. 
- Pořád jsme v bodu `x[0] = a` a zkontrolujeme variaci funkce na novém intervalu mezi x[0] a x[2]
- Pokud je potřeba, přidáváme další body. 
- Pokud je variace funkce pod stanovou hranicí, přesuneme se na následující bod. 

Takovýto proces se špatně ukládá do seznamu, ale úplně dobře se dá realizovat na spojovaném seznamu. To také uděláme:
...
- Aby jsme zachytili případ, kdy hodnoty funkce na okrajích intervalu jsou blízké, ale uvnitř intervalu se funkce výrazně mění (např. sin(x) na intervalu 0 - 180°), testujeme diference také mezi prostřední hodnotou a hodnotami na krajích. 
- Funkci přijímáme jako řetězec a pro její vyhodnocení používáme funkci `eval`.
- Pro obsluhu spojovaného seznamu potřebujeme jedinou metodu`insert_next(node, x)`, která vloží nový uzel za zadaný uzel seznamu. Ostatní metody dělají zřejmé věci.
- Pro nakreslení bodů používáme modul `matplotlib`.
...
1. **LSS množiny: destruktivní průnik** - máte vytvořit LSS, obsahující destruktivní průnik dvou utříděných LSS. Destruktivní znamená, že výsledný seznam vytváříte z uzlů vstupních seznamů a nevytvářite / nekopírujete žádné uzly. 
2. **Kontrola uzávorkování - jeden typ závorek** - Proč potřebujeme zásobník a nestačí obyčejné počítadlo? (Ale stačí neobyčejné počítadlo.)
